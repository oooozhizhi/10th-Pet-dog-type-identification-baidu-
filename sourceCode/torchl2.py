...
train_feat=train_feat1[0][0:8]
for i in range(10,23350,10):
    print(i)
    train_feat = np.concatenate((train_feat,train_feat1[0][i:i+8]), 0)

train_feat = np.concatenate((train_feat,train_feat1[0][23350:]), 0)
   
test_feat=test_feat1[0][0:8]
for i in range(10,13230,10):
    print(i)
    test_feat = np.concatenate((test_feat,test_feat1[0][i:i+8]), 0)
...
print('Feature:', test_feat.shape)
print('Feature:', trainlabel.shape)
...
train_logs = [[], [], [], []]
for train_index, test_index in skf.split(train_feat, trainlabel):
    ...
    #train_set = Arrayloader(X_train, y_train.tolist())
    X_train = torch.from_numpy(X_train)
    y_train = torch.from_numpy(np.array(y_train.tolist(),dtype=np.int64))
    train_set = Data.TensorDataset(data_tensor=X_train, target_tensor=y_train)  
    train_loader = torch.utils.data.DataLoader(train_set, batch_size = 128, shuffle=True, num_workers=4)
    print('train load')
    X_test = torch.from_numpy(X_test)
    y_test = torch.from_numpy(np.array(y_test.tolist(),dtype=np.int64))
    val_set = Data.TensorDataset(data_tensor=X_test, target_tensor=y_test)  
    ...
    for epoch in range(epochsize):
        ...
        for data in train_loader:
            dta_x, dta_y = data
            dta_x, dta_y = Variable(dta_x), Variable(dta_y)
